    ...
    public partial class UserDash : Form
    {
        ...
        private void addCCBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                        Con.Open();
                        string query = "update UDataTbl set CCName ='" + NameCC.Text + "', CCNum = '" + CCNum.Text + "', CCExp = '" + ExpirDate.Text + "'," +
                        " CCAdd = '" + cardyes + "' where UserEmail = '" + UEmail.Text + "';";
        ...
        private void prodBuyPnl_Paint(object sender, PaintEventArgs e)
        {
                ...
                if (a == 1)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            totprice = prodprice * inputqty;
                            newprodqty = oldprodqty - inputqty;
                            string query = "insert into OrdersTbl values('" + UEmail.Text + "','" + UFName.Text + "','" + AddressBox.Text + "','" 
                                + ProductN.Text + "','" + ProductP.Text + "','" + QtyBox.Text + "','Manually','" + totprice + "')";
                            SqlCommand cmd = new SqlCommand(query, Con);
                            cmd.ExecuteNonQuery();
                            string queryy = "update ProductsTbl set Quantity ='" + newprodqty + "' where Product = '" + ProductN.Text + "';";
                            SqlCommand cmdr = new SqlCommand(queryy, Con);

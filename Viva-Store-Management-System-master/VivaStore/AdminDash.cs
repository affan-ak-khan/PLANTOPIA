...
namespace VivaStore
{
    public partial class AdminDash : Form
    {
        ...
        private void AdminDash_Load(object sender, EventArgs e)
        {
            ...
            // Ensure proper settings for flowLayoutPanel3
        ...
        private void visibleAdminComponent()
        {
            ...
            addProdPanel.Location = new Point(480, 230);
        ...
        private void getInfoData()
        {
            ...
            sdt4.Fill(dr4);
            BudgetLabel.Text = "Rs" +dr4.Rows[0][0].ToString();
        }
        //Function Admin Data
        ...
        public void GenerateDynamicProductsControl()
        {
            ...
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        if (productCounter % 5 == 0 && productCounter != 0)
                        {
                            // Add an empty Panel to force a new row
                            Panel newRowPanel = new Panel();
                            newRowPanel.Size = new Size(flowLayoutPanel3.Width, 1); // 1px height to force wrap
                            flowLayoutPanel3.Controls.Add(newRowPanel);
                        }

                        AProdControl prodControl = new AProdControl();
                        MemoryStream ms = new MemoryStream((byte[])row["Image"]);
                        prodControl.Icon = new Bitmap(ms);
                        prodControl.ProductID = "ID: " + row["IDP"].ToString();
                        prodControl.ProductsName = "Product: " + row["Product"].ToString();
                        prodControl.ProductPrice = "Price: " + row["Price"].ToString() + " €";
                        prodControl.ProductQty = "Quantity: " + row["Quantity"].ToString();
                        flowLayoutPanel3.Controls.Add(prodControl);
        ...
        private void ListAdminPanel_Paint(object sender, PaintEventArgs e)
        {
